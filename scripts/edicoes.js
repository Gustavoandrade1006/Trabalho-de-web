...
function filtrarCopas(tipo, texto) {
    if (tipo == "ano") {
        return copas.filter(copa => copa.ano == texto.trim());
    } else {
        return copas.filter(copa => copa.campeao.toLowerCase() == texto.toLowerCase());
    }
}

let navFiltro = document.getElementById("nav-filtro");
let indiceFiltro = 0;
let copasFiltradas = [];

let formularioFiltro = document.getElementById("form-filtro");

formularioFiltro.addEventListener("submit", function (event) {
    event.preventDefault();
    tipo = document.getElementById("select-filtro");
    texto = document.getElementById("input-filtro");
    copasFiltradas = filtrarCopas(tipo.value, texto.value);
    console.log(copasFiltradas);
    if (copasFiltradas.length > 0) {
        localStorage.setItem("filtro", true);
        navFiltro.style.setProperty("display", "flex", "important");
        navFiltro.classList.add("d-flex", "flex-column", "align-items-center");
        indiceFiltro = 0;
        setTimeout(() => {
            irCopa();
        }, 300);
...
const infoFiltro = document.getElementById("filtro-info");
function irCopa() {
    let copaAtual = copasFiltradas[indiceFiltro];
    infoFiltro.innerText = `Copa ${indiceFiltro + 1} de ${copasFiltradas.length}`;
    let copaCard = document.getElementById(copaAtual.id);
    copaCard.scrollIntoView({ behavior: "smooth", block: "center" });
}
...
function antesCopa() {
    if (indiceFiltro > 0) {
        ...
        indiceFiltro = copasFiltradas.length - 1;
    }
    irCopa()
}
function proximoCopa() {
    if (indiceFiltro < copasFiltradas.length - 1) {
        indiceFiltro++;
    } else {
        indiceFiltro = 0;
    }
...
function limparFiltro() {
    navFiltro.classList.remove("d-flex", "flex-column", "align-items-center");
    navFiltro.style.setProperty("display", "none", "important");
